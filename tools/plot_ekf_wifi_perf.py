#!/usr/bin/env python3
"""
plot_ekf_wifi_perf.py — Load CSV from Wi-Fi EKF test and compute/plot stats.
Outputs:
  - ekf_wifi_offset.png : offset vs time
  - ekf_wifi_drift.png  : drift(ppb) vs time
  - ekf_wifi_hist.png   : offset histogram with mean/median/std
  - ekf_wifi_allan.png  : Allan deviation vs tau (log-log)
...
def load_csv(path: str):
    ...
    return np.array(t), np.array(off), np.array(drift), np.array(z_meas), np.array(had)
...
def main():
    ...
    ap.add_argument("csv", help="CSV produced by EKF Wi-Fi test")
    ap.add_argument("--warmup", type=float, default=2.0, help="Seconds to discard at start for steady-state stats")
    ap.add_argument("--prefix", default="ekf_wifi_", help="Output file prefix")
    args = ap.parse_args()
    ...
    t, off, drift, z_meas, had = load_csv(args.csv)
    ...
    plt.xlabel("Time (s)"); plt.ylabel("Offset (µs)"); plt.title("EKF Offset vs Time")
    plt.grid(True, which='both')
    plt.savefig(output_prefix + "offset.png", dpi=160, bbox_inches="tight"); plt.close()

    plt.figure()
    plt.plot(t, drift, linewidth=1.0)
    plt.xlabel("Time (s)"); plt.ylabel("Drift estimate (ppb)"); plt.title("EKF Drift vs Time")
    plt.grid(True, which='both')
    ...
    plt.xlabel("Offset (µs)"); plt.ylabel("PDF"); plt.title("EKF Offset histogram (steady-state)")
    plt.legend()
    plt.grid(True, which='both')
    ...
    if len(adev):
        ...
        plt.xlabel("Tau (s)"); plt.ylabel("Allan deviation (fractional)"); plt.title("EKF Allan deviation")
        plt.grid(True, which='both')
