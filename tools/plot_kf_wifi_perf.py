#!/usr/bin/env python3
"""
plot_kf_wifi_perf.py — Load CSV from Wi-Fi KF test and compute/plot stats.
Outputs:
  - kf_wifi_offset.png : offset vs time
  - kf_wifi_drift.png  : drift(ppb) vs time
  - kf_wifi_hist.png   : offset histogram with mean/median/std
  - kf_wifi_allan.png  : Allan deviation vs tau (log-log)
...
def load_csv(path: str):
    ...
    with open(path, 'r') as f:
        ...
        for row in r:
            ...
            z.append(float(row['z_meas_s']))
    return np.array(t), np.array(off), np.array(drift), np.array(had), np.array(z)
...
def main():
    ...
    ap.add_argument("csv", help="CSV produced by Wi-Fi test (PERF_OUT)")
    ap.add_argument("--warmup", type=float, default=2.0, help="Seconds to discard at start for steady-state stats")
    ap.add_argument("--prefix", default="kf_wifi_", help="Output file prefix")
    args = ap.parse_args()
    ...
    t, off, drift, had, z = load_csv(args.csv)
    ...
    plt.xlabel("Time (s)"); plt.ylabel("Offset (µs)"); plt.title("Offset vs Time")
    plt.grid(True, which='both')
    plt.savefig(output_prefix + "offset.png", dpi=160, bbox_inches="tight"); plt.close()

    plt.figure()
    plt.plot(t, drift, linewidth=1.0)
    plt.xlabel("Time (s)"); plt.ylabel("Drift estimate (ppb)"); plt.title("Drift vs Time")
    plt.grid(True, which='both')
    ...
    plt.xlabel("Offset (µs)"); plt.ylabel("PDF"); plt.title("Offset histogram (steady-state)")
    plt.legend()
    plt.grid(True, which='both')
    ...
    if len(adev):
        ...
        plt.xlabel("Tau (s)"); plt.ylabel("Allan deviation (fractional)"); plt.title("Allan deviation")
        plt.grid(True, which='both')
