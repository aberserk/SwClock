#!/usr/bin/env python3
"""
plot_aekf_wifi_perf.py — Load CSV from Wi-Fi AEKF test and compute/plot stats.
Outputs:
  - aekf_wifi_offset.png : offset vs time
  - aekf_wifi_drift.png  : drift(ppb) vs time
  - aekf_wifi_hist.png   : offset histogram with mean/median/std
  - aekf_wifi_allan.png  : Allan deviation vs tau (log-log)
  - aekf_wifi_adapt.png  : adaptation parameters (R_adapt, Q00, Q11) vs time
...
def main():
    ...
    ap.add_argument("csv", help="CSV produced by AEKF Wi-Fi test")
    ap.add_argument("--warmup", type=float, default=2.0, help="Seconds to discard at start for steady-state stats")
    ap.add_argument("--prefix", default="aekf_wifi_", help="Output file prefix")
    args = ap.parse_args()
    ...
    plt.xlabel("Time (s)"); plt.ylabel("Offset (µs)"); plt.title("AEKF Offset vs Time")
    plt.grid(True, which='both')
    plt.savefig(output_prefix + "offset.png", dpi=160, bbox_inches="tight"); plt.close()

    plt.figure()
    plt.plot(t, drift, linewidth=1.0)
    plt.xlabel("Time (s)"); plt.ylabel("Drift estimate (ppb)"); plt.title("AEKF Drift vs Time")
    plt.grid(True, which='both')
    ...
    plt.xlabel("Offset (µs)"); plt.ylabel("PDF"); plt.title("AEKF Offset histogram (steady-state)")
    plt.legend()
    plt.grid(True, which='both')
    ...
    if len(adev):
        ...
        plt.xlabel("Tau (s)"); plt.ylabel("Allan deviation (fractional)"); plt.title("AEKF Allan deviation")
        plt.grid(True, which='both')
        plt.savefig(output_prefix + "allan.png", dpi=160, bbox_inches="tight"); plt.close()

    # Adaptation parameters plot (unique to AEKF)
    plt.figure(figsize=(12, 8))
    
    plt.subplot(3, 1, 1)
    plt.plot(t, R_adapt, linewidth=1.0, color='red')
    plt.xlabel("Time (s)"); plt.ylabel("R_adapt"); plt.title("AEKF Measurement Noise Adaptation")
    plt.grid(True, which='both')
    
    plt.subplot(3, 1, 2)
    plt.plot(t, Q00, linewidth=1.0, color='blue')
    plt.xlabel("Time (s)"); plt.ylabel("Q00 (offset)"); plt.title("AEKF Process Noise Q00 (Offset)")
    plt.grid(True, which='both')
    plt.yscale('log')
    
    plt.subplot(3, 1, 3)
    plt.plot(t, Q11, linewidth=1.0, color='green')
    plt.xlabel("Time (s)"); plt.ylabel("Q11 (drift)"); plt.title("AEKF Process Noise Q11 (Drift)")
    plt.grid(True, which='both')
    plt.yscale('log')
