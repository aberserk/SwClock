...
TEST(EKFServo_Wifi, HandlesWifiNoise) {
    SwClock* sw = swclock_create(); ASSERT_NE(sw,nullptr);
    ExtendedKalmanFilter* ekf = ekf_create(); ASSERT_NE(ekf,nullptr);
    ekf_init(ekf, 1e-8, 2e-6);
    ...
    std::string out_csv = output_dir + "/ekf_wifi_perf.csv";
    
    const int iters = 900;
    const int tick_ms = 10;

    std::ofstream csv(out_csv);
    csv << "t_s,offset_s,drift_ppb,z_meas_s,had_meas\n";
    ...
    double drift_ppb = std::fabs(ekf_get_drift_ppb(ekf));
    EXPECT_LT(off_ms, 6.0) << "EKF offset under harsh Wi-Fi should be < 6 ms";
    EXPECT_LT(drift_ppb, 500000.0);
    ...
    std::string plot_cmd = "python3 tools/plot_ekf_wifi_perf.py " + csv_path;
