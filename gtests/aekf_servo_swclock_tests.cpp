...
TEST(AEKF, Converges){ SwClock* sw=swclock_create(); ASSERT_NE(sw,nullptr);
    AdaptiveExtendedKalmanFilter* ekf=aekf_create(); ASSERT_NE(ekf,nullptr); aekf_init(ekf,1e-8,1e-6);
    ...
    for(int i=0;i<450;++i){ double z=measure_offset_s(sw, master_now_ns(t0,start_ns)); (void)aekf_update(ekf,z,0.01);
        struct timex tf{}; tf.modes=ADJ_FREQUENCY; tf.freq=ppb_to_freq_fixed(aekf_get_drift_ppb(ekf)); sw_adjtimex(sw,&tf);
        struct timex to{}; to.modes=ADJ_OFFSET; to.offset=(long)std::llround(aekf_get_offset(ekf)*1e6); sw_adjtimex(sw,&to); msleep(10); }
    double off_ms=fabs(measure_offset_s(sw, master_now_ns(t0,start_ns)))*1e3; EXPECT_LT(off_ms,1.0); EXPECT_LT(fabs(aekf_get_drift_ppb(ekf)),400.0);
    aekf_destroy(ekf); swclock_destroy(sw); }
