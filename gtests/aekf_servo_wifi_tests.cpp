...
TEST(AEKFServo_Wifi, AdaptsUnderWifiNoise) {
    SwClock* sw = swclock_create(); ASSERT_NE(sw,nullptr);
    AdaptiveExtendedKalmanFilter* e = aekf_create(); ASSERT_NE(e,nullptr);
    aekf_init(e, 1e-8, 2e-6);
    ...
    std::string out_csv = output_dir + "/aekf_wifi_perf.csv";
    ...
    for (int i=0;i<iters;++i){
        ...
        if (have){
            (void)aekf_update(e, z_meas, dt);
            last_tp = nowtp;

            struct timex tf{}; tf.modes=ADJ_FREQUENCY;
            tf.freq = ppb_to_freq_fixed(aekf_get_drift_ppb(e));
            sw_adjtimex(sw, &tf);
            struct timex to{}; to.modes=ADJ_OFFSET;
            to.offset = (long)std::llround(aekf_get_offset(e)*1e6);
            sw_adjtimex(sw, &to);
        }

        double t_s = std::chrono::duration<double>(nowtp - t0).count();
        double off_s = measure_offset_s(sw, master_now());
        csv << t_s << "," << off_s << "," << aekf_get_drift_ppb(e) << ","
            << (have?z_meas:0.0) << "," << (have?1:0) << ","
            << aekf_get_R_adapt(e) << "," << aekf_get_Q_offset(e) << "," << aekf_get_Q_drift(e) << "\n";
    ...
    double drift_ppb = std::fabs(aekf_get_drift_ppb(e));
    EXPECT_LT(off_ms, 20.0) << "AEKF offset under harsh Wi-Fi should be < 20 ms";
    EXPECT_LT(drift_ppb, 1000000.0);

    aekf_destroy(e); swclock_destroy(sw);
    ...
    std::string plot_cmd = "python3 tools/plot_aekf_wifi_perf.py " + csv_path;
